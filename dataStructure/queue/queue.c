...
void printQueue(LIST *L)
{
    ...
    else
    {
        NODE *temp;
        printf("\nLIST = ");
        for (temp = L->head; temp != NULL; temp = temp->next) // print all the contents
        {
            printf("%d ", temp->value);
        }
    }
...
{
    NODE *new = (NODE *)malloc(sizeof(NODE)); // create a new node and initialize its value
    new->value = data;
    new->next = NULL;
    return new;
}
...
LIST *createQueue()
{
    LIST *L = (LIST *)malloc(sizeof(LIST)); // create new list where head and tail is nulll
    L->head = NULL;
    L->tail = NULL;
    return L;
}
...
*/
void enqueue(LIST *L, NODE *node) // add at tail
{
...
{
    int val;
    if (isEmpty(L)) // don't dequeue if list is empty
    {
        printf("\nList is empty!\n");
        return -1;
    }
